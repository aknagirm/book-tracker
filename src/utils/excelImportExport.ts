  ...
  if (wb.SheetNames.includes('Books')) {
    ...
    rows.forEach((row, i) => {
      ...
      importedBooks.push({
        ...
        actualPrice: toNum(row['Printed Price'] ?? row['Printed Price (₹)'] ?? row['Actual Price (₹)']),
        discountedPrice: toNum(row['Discounted Price'] ?? row['Discounted Price (₹)']),
